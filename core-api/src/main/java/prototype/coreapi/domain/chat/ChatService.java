    ...
    public Mono<Chat> findById(Long id) {
        return chatRepository.findById(id)
                .switchIfEmpty(Mono.error(
                        new BusinessException(ErrorCode.NO_SUCH_CONTENT_VALUE, "대화")
                ));
    }
    ...
    public Mono<Chat> createChat(Long memberId) {
        ...
        return Mono.zip(memberMono, countMono)
                .flatMap(tuple -> {
                    ...
                    String title = today.format(DateTimeFormatter.ofPattern("yyyy년 MM월 dd일")) + "의 채팅";
                    if (count > 0) {
                        title += " (" + (count + 1) + ")";
                    }
